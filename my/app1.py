...
"""
    Generovani 15 sad glyphu od kazdeho druhu, pocitadlo 
    nahodne glyphy done
    glyphB nahodne/od nuly - od nuly done
    vetsi vlevo, mensi vpravo done
    neuronka
    glyphy od studentu viz github
    modelovani log stupnici, gamma korekce
    scipy optimalizacni algoritmus 
"""
...
def horizontal_line(x: float, canvas: mg.Canvas) -> None:
    line_start = (mg.lerp(x, canvas.xcenter, canvas.xleft), canvas.ycenter)
    line_end = (mg.lerp(x, canvas.xcenter, canvas.xright), canvas.ycenter)
    canvas.line(line_start, line_end, width=5.0, linecap="round")
...
def simple_scaled_star(x: float, canvas: mg.Canvas) -> None:
    ...
    canvas.line(vertices, width=2.0, linecap="round", color="indigo")


def simple_scaled_circle(x: float, canvas: mg.Canvas) -> None:
    radius = mg.lerp(x, 0.01, canvas.ysize / 2)
    canvas.circle(canvas.center, radius, color="navy", style="stroke", width="15p")
...
def render_png(glyph_type: str, x: float, *, dpi: float = 96.0) -> bytes:
    if glyph_type not in glyphs:
        raise KeyError(f"Unknown glyph_type '{glyph_type}'")

    print(f"[RENDER] Generating glyph '{glyph_type}' with value {x:.1f}")
    
    params = mg.CanvasParameters(padding_horizontal='5%', padding_vertical='5%', background_color='white')
    canvas = mg.Canvas([800, 800], canvas_parameters=params)
    ...
    if not data:
        print("[ERROR] Failed to encode image to PNG")
        return b""
    return data.bytes()
...
class GlyphWidget(QtWidgets.QWidget):
    ...
    def update_image(self):
        print(f"[DEBUG] Rendering {self.glyph_type} with value {self.value:.1f}")
        png_data = render_png(self.glyph_type, self.value)

        if not png_data:
            print("[ERROR] PNG data is empty!")
            return
        ...
        if success:
            ...
            self.image_label.setFixedSize(pixmap.size())  # 💡 DŮLEŽITÉ
            print(f"[DEBUG] Pixmap size: {pixmap.size()}")
        else:
            print("[ERROR] Failed to load pixmap from PNG data")

        self.value_label.setText(f"DEBUG: {self.value:.1f}")
